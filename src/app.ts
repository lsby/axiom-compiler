...
let 表达式G = new 运算('+', [new 运算('*', [new 参数引用('a'), new 参数引用('b')]), new 数(10)])
...
// 演示图片生成
console.log('\n正在生成图片...')
await Latex渲染器.渲染并保存(latex代码, './data/formula_g.svg')
await Latex渲染器.渲染并保存(latex代码, './data/formula_g.png')
console.log('图片已保存为: formula_g.svg 和 formula_g.png')

// 演示带括号和除法的渲染
let 复杂表达式 = new 运算('/', [
  new 运算('+', [new 数(1), new 参数引用('x')]),
  new 运算('-', [new 数(10), new 参数引用('y')]),
])
let 复杂Latex = 数学渲染器.渲染为Latex(复杂表达式)
console.log('\n复杂表达式纯文本:', 数学渲染器.渲染为文本(复杂表达式))
console.log('复杂表达式 LaTeX:', 复杂Latex)

await Latex渲染器.渲染并保存(复杂Latex, './data/formula_complex.png')
console.log('复杂表达式图片已保存为: formula_complex.png')
