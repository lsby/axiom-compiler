...
import { 操作, 计算操作返回值类型 } from '../base/operation.js'
...
> extends 调用<算子类型, 参数类型> {
  ...
  >(
    符号名: S,
    替换物: R,
  ): 表达式<Omit<计算数据符号映射<参数类型>, S> & 计算表达式符号映射<R>, 计算操作返回值类型<算子类型>> {
    return new 函数(this.我的算子, this.我的参数.代换(符号名 as any, 替换物) as any) as any
  }
  ...
  public 派生<const N extends string, const S extends string>(
    ...
    let 体 = 变换逻辑(this)
    let 新算子 = 算子.从表达式创建(名称, 参数符号 as any, 体 as any)
...
export class 不动点函数<const 参数类型 extends 任意的数据, 返回值> extends 表达式<计算数据符号映射<参数类型>, 返回值> {
  ...
  public override 代换<
    S extends Extract<keyof 计算数据符号映射<参数类型>, string> | (string & {}),
    R extends 表达式<any, S extends keyof 计算数据符号映射<参数类型> ? 计算数据符号映射<参数类型>[S] : any>,
  >(符号名: S, 替换物: R): 表达式<Omit<计算数据符号映射<参数类型>, S> & 计算表达式符号映射<R>, 返回值> {
    return new 不动点函数(
      this.我的不动点,
